...
// Initialize vendors session if not already set
...
// Check and add Admin(dairy)
$dairyExists = false;
foreach ($_SESSION['vendors'] as $vendor) {
    if ($vendor['vendor_name'] === 'Admin(dairy)') {
        $dairyExists = true;
        break;
    }
}
...
$fruit = [
    ...
    ['name'=>'Mango (Mangga)','price'=>5,'unit'=>'per kg','description'=>'Juicy and sweet, grown locally in many varieties like Harumanis and Chokanan.','image'=>'https://www.google.com/imgres?q=ripe%20mango%20images&imgurl=https%3A%2F%2Fcdn.pixabay.com%2Fphoto%2F2023%2F09%2F29%2F07%2F58%2Fmango-8283268_640.jpg&imgrefurl=https%3A%2F%2Fpixabay.com%2Fimages%2Fsearch%2Fripe%2520mango%2F&docid=zGMy3OlOc4VldM&tbnid=BO5LtqpznBF4OM&vet=12ahUKEwjX8Lmcm9yMAxXe4zgGHdcFOngQM3oECBcQAA..i&w=640&h=427&hcb=2&ved=2ahUKEwjX8Lmcm9yMAxXe4zgGHdcFOngQM3oECBcQAA']
    ];

// Check and add Admin(fruit)
$fruitExists = false;
foreach ($_SESSION['vendors'] as $vendor) {
    if ($vendor['vendor_name'] === 'Admin(fruit)') {
        $fruitExists = true;
        break;
    }
}
...
// Check and add Admin(vegetables)
$vegetableExists = false;
foreach ($_SESSION['vendors'] as $vendor) {
    if ($vendor['vendor_name'] === 'Admin(vegetables)') {
...
// Check and add Admin(forestry)
$forestryExists = false;
foreach ($_SESSION['vendors'] as $vendor) {
    if ($vendor['vendor_name'] === 'Admin(Forestry)') {
        $forestryExists = true;
        break;
    }
}
...
// Check and add Admin(fish)
$fishExists = false;
foreach ($_SESSION['vendors'] as $vendor) {
    if ($vendor['vendor_name'] === 'Admin(Fish)') {
        $fishExists = true;
        break;
    }
}
...
// Check and add Admin(miscellaneous)
$miscellaneousExists = false;
foreach ($_SESSION['vendors'] as $vendor) {
    if ($vendor['vendor_name'] === 'Admin(Miscellaneous)') {
...
if (!$miscellaneousExists) {
    $_SESSION['vendors'][] = [
        'vendor_name' => 'Admin(Miscellaneous)',
...
<body>
    <div class="container">
        <h1>Welcome to Our Vendor Marketplace</h1>
        <div class="vendor-container">
            <?php if (count($_SESSION['vendors']) > 0): ?>
                <div class="vendor-list">
                    <?php foreach ($_SESSION['vendors'] as $vendor_id => $vendor): ?>
                        <div class="vendor-card">
                            <h2 class="vendor-name"><?= htmlspecialchars($vendor['vendor_name']) ?></h2>
                            <div class="vendor-actions">
                                <a href="edit_vendor.php?vendor_id=<?= $vendor_id ?>" class="btn edit-btn">Edit Vendor</a>
                                <a href="?delete_vendor_id=<?= $vendor_id ?>" class="btn delete-btn" onclick="return confirm('Are you sure you want to delete this vendor?')">Delete Vendor</a>
                            </div>
                            <div class="products">
                                <h3>Products:</h3>
            ...
            <?php endif; ?>
            <a href="register_vendor.php" class="btn add-vendor-btn">Add New Vendor</a>
        </div>
    </div>
</body>
</html>
