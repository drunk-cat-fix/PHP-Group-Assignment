...
require_once 'service/Products.php';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<p><?= htmlspecialchars($vendor['shop_address'] . ', ' . $vendor['shop_city'] . ', ' . $vendor['shop_state']) ?></p>
<p><strong>Email:</strong> <?= htmlspecialchars($vendor['vendor_email']) ?></p>
<p><strong>Tier:</strong> <?= htmlspecialchars($vendor['vendor_tier']) ?></p>
<p><strong>About:</strong> <?= nl2br(htmlspecialchars($vendor['vendor_desc'])) ?></p>
<?php if (!empty($vendor['vendor_profile'])): ?>
    <img src="data:image/jpeg;base64,<?= base64_encode($vendor['vendor_profile']) ?>" alt="Vendor Profile" style="max-width: 150px;">
<?php endif; ?>
